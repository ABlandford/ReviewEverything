...
router.post("/submitReview", function(req, res) {
  ...
  RR.findOne(
    { movieTitle: req.body.movieTitle, username: req.body.username },
    (err, review) => {
      if (!review) {
        var r = new RR({
        ...
        res.send({
          error: true,
          errorMessage:
            "You already have a review of this movie. Check your reviews to edit or delete your current review."
        });
      }
    }
  );
  ...
  if (req.body.username != null && req.body.password != null) {
    ...
    User.findOne({ username: req.body.username }, function(err, user) {
      if (!user) {
        ...
        res.send({ status, statusMessage, locked: user.locked });
      } else if (user) {
        ...
        bcrypt.compare(req.body.password, user.password).then(res2 => {
          if (res2 == true) {
            status = !status;
            res.send({ status, user,locked: user.locked });
          } else {
            let statusMessage = "The PASSWORD you entered was incorrect.";
            let passwordFail = true;
            res.send({ status, statusMessage, passwordFail,locked: user.locked });
          }
        });
      }
    });
    ...
    let status = false;
    let statusMessage =
      "You're submitting nothing. Please enter something into the text boxes before submitting.";
    res.send({ status, statusMessage,locked: user.locked });
  }
});
...
router.put("/setPass", function(req, res) {
    ...
    User.findOne({ email: req.body.email }, (err, user) => {
        ...
        bcrypt.hash(user.password, saltRounds).then((hash) => {
            ...
            user.save((err, savedUser) => {
              if (err) return console.log(err);
              console.log(savedUser.locked + " Sorry");
...
router.put("/hashedUsername", function(req, res) {
  User.findOne({ email: req.body.email }, (err, user) => {
    bcrypt.hash(user.username, saltRounds).then((hash) => {
        user.username = hash
    
        res.send({hashuser: user.username})
    });
});
});

router.put("/lockAccount", function(req, res) {
